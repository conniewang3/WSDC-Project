...
class DanceScraper:
    ...
    def parse_response(self, response):
        ...
        for location in locations:
            if any(x in location for x in west_coast):
                counts['West Coast'] += 1
            elif any(x in location for x in midwest):
                counts['Midwest'] += 1
            elif any(x in location for x in east_coast):
                counts['East Coast'] += 1
            elif any(x in location for x in other):
                counts['Other'] += 1
                continue
            else: 
                counts['Europe'] += 1
    ...
    def run(self):
        raw_data = pkl.load(open('raw_data.p', 'rb'))
        ...
        for data in raw_data:
            ...
            # Only accept competitors with at least 1 newcomer/nov comp
            if (
                parsed['nov_count'] > 0 or
                parsed['new_count'] > 0
                ):
                scraped_info = scraped_info.append(parsed, ignore_index=True)
        
        scraped_info.to_csv('data.csv')
