...
To answer these questions, I looked at only dancers who pointed out of Novice after February 2015 (when the old rules took effect). I wasn't able to find copies of the old rules so I don't know when exactly the point requirement became 15 -- if you have this information, I'd love to know!
...
new_novice <- data %>%
  filter(as.Date(paste(nov_end, '01'), '%B %Y %d') > as.Date('2015-02-01'))
...
Of people who were previously competing in Intermediate, **`r must_nov`** now *must* compete in Novice, and **`r can_nov`** now *have the option* to compete in Novice. This represents **`r percents[1]`%** and **`r percents[2]`%** of current Novice dancers or **`r percents[3]`%** and **`r percents[4]`%** of current Intermediate dancers. 
...
For good measure, I also broke down the mean number of Novice events for 15-point-Novices by region. It's about the same across regions: 4-ish events. 
...
Significance testing by region, we see that dancers in the Other region (Asia/Australia/Brazil) with exactly 15 Novice points **place less frequently in Intermediate**. The is also the trend in 3 of the other 4 regions (East Coast, Europe, Midwest), although the differences there are not significant. 
...
percent_byregion <- left_join(totals, percent_byregion, by = c("nov_start", "region"))
filled <- percent_byregion %>% 
  fill(count)
percent_byregion <- filled %>%
  mutate(percent = count / total_count * 100) %>%
  select(nov_start, region, percent)
...
# Build plots
all_regions <- ggplot(percent_byregion, aes(x=nov_start, y=percent, fill=region)) +
...
combined_US <- ggplot(percent_byregion2, aes(x=nov_start, y=percent, fill=region)) +
  geom_bar(position='stack', stat='identity') +
  coord_cartesian(xlim = as.Date(c('2013-01-01', '2017-12-01'))) +
  theme(axis.title.x=element_blank(), axis.title.y=element_blank()) +
  labs(fill = 'Region')
...
count_byregion <- count_byregion %>%
  ...
  mutate(months = as.integer(nov_start - as.Date('2014-01-01'))/30) %>%
  filter(months > 0) %>%
  ungroup() %>%
